...
export default function OrderPage() {
  ...
  const [basePrice, setBasePrice] = useState(PRICE_PER_PIECE)
  ...
  useEffect(() => {
    fetch('/api/account/pricing').then(r => r.json()).then(d => {
      if (d.price_per_piece) setPricePerPiece(d.price_per_piece)
      if (d.base_price) setBasePrice(d.base_price)
    }).catch(() => {})
            ...
            <div>
              ...
              {pricePerPiece < basePrice ? (
                <div className="mt-2 flex items-center gap-2 flex-wrap">
                  <p className="text-sm line-through" style={{ color: '#b09090' }}>ชิ้นละ {basePrice} บาท</p>
                ...
                <p className="text-2xl font-black mt-2" style={{ color: '#4a2728' }}>ชิ้นละ {pricePerPiece} บาท</p>
              )}
            </div>
